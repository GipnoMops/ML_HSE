...
class WordAligner(BaseAligner):
    ...
    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[np.array]:
        ...
        posteriors = []
        for sentence in parallel_corpus:
            cur = self.translation_probs[sentence.source_tokens[:, np.newaxis], sentence.target_tokens[np.newaxis, :]]
            posteriors.append(cur / cur.sum(axis=0))
        return posteriors
    ...
    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]) -> float:
        ...
        for i, sentence in enumerate(parallel_corpus):
            table = self.translation_probs[sentence.source_tokens[:, np.newaxis],
            sentence.target_tokens[np.newaxis, :]]
            table /= posteriors[i] * len(sentence.source_tokens)
            elbo += np.sum(np.where(posteriors[i] > 0, posteriors[i] * np.log(table), 0))
        return elbo
    ...
    def _m_step(self, parallel_corpus: List[TokenizedSentencePair], posteriors: List[np.array]):
        ...
        for i, sentence in enumerate(parallel_corpus):
            idx = (sentence.source_tokens[:, np.newaxis], sentence.target_tokens[np.newaxis, :])
...
#        for i in range(self.num_iters):
    ...
    def align(self, sentences):
        ...
        for i, sentence in enumerate(sentences):
            aligments = np.argmax(posteriors[i], axis=0) + 1
            indx = range(1, len(sentence.target_tokens) + 1)
            result.append(list(zip(aligments, indx)))
        return result
